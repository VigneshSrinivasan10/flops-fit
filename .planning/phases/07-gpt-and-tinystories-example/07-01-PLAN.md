...
must_haves:
  truths:
    ...
    - "GPT instance satisfies library contract: `gpt.num_params()` returns a positive integer"
  ...
  artifacts:
    ...
    - path: "src/flops_fit/examples/gpt.py"
      provides: "GPT model with num_params() contract method"
      contains: "def num_params(self)"
...
<objective>
Create the `flops_fit.examples` package by moving GPT to examples, adding the `num_params()` contract method, creating the TinyStories dataset wrapper, and maintaining backward compatibility.
...
Output: `src/flops_fit/examples/` package with GPT (+ num_params() method), TinyStoriesDataset wrapper, and backward-compat re-exports in model.py.
</objective>
...
<task type="auto">
  <name>Task 1: Create examples package with GPT (moved + num_params() added)</name>
  <files>
    src/flops_fit/examples/__init__.py
    src/flops_fit/examples/gpt.py
    src/flops_fit/model.py
  ...
  <action>
    ...
    3. In `src/flops_fit/examples/gpt.py`, add the `num_params()` method to the `GPT` class (primary library contract). Place it immediately after `count_parameters()` (line ~387 in original):
       ```python
       ...
       NOTE: The contract validator in model_factory.py checks for `num_params()` — NOT `n()`.
       Do NOT add `n()` instead of `num_params()`. You may optionally add `n = num_params` as an
       alias after the method definition if desired, but `num_params()` must be the primary method.
  ...
  <verify>
    Run: `python -c "from flops_fit.model import GPT, GPTConfig; m = GPT(GPTConfig(d_model=64, num_layers=2, num_heads=2)); print(m.num_params())"` — should print a positive integer.
  ...
  <done>
    `from flops_fit.model import GPT` and `from flops_fit.examples import GPT` both work. GPT instance has `num_params()` returning positive int. Existing model tests pass.
  </done>
</task>
...
Run contract check: `python -c "from flops_fit.examples import GPT, GPTConfig; m = GPT(GPTConfig(d_model=64, num_layers=2, num_heads=2)); assert hasattr(m, 'num_params'), 'num_params() missing'; print(m.num_params())"`
</verification>
...
3. `GPT(GPTConfig(d_model=64, num_layers=2, num_heads=2)).num_params()` returns a positive integer.
