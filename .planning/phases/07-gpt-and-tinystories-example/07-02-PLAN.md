...
must_haves:
  ...
  key_links:
    - from: "src/flops_fit/examples/example_programmatic.py"
      ...
      via: "import flops_fit; flops_fit.find_optimal(...)"
...
<objective>
Create two runnable example scripts showing how users integrate `flops_fit.find_optimal()` with GPT + TinyStories, plus tests that verify the examples work without network access.

Purpose: These scripts are the primary user-facing deliverable of Phase 7 — the "how do I use this?" reference. Tests guard against regressions using mock HuggingFace to keep CI fast and network-free.

Output: `example_programmatic.py`, `example_cli_wrapper.py`, and `tests/test_examples.py`.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    def make_model_factory(num_layers: int = 4, num_heads: int = 4):
        ...
        Returns:
            Callable: (d_model: int) -> GPT instance with n() method.
        """
    ...
    def run(use_real_data: bool = False, output_dir: str = "outputs/gpt_tinystories"):
        ...
        Args:
            use_real_data: If True, downloads TinyStories from HuggingFace.
    ...
    def main():
        ...
        parser.add_argument(
            "--real",
            action="store_true",
            help="Use real TinyStories data (requires internet). Default: synthetic mock data.",
        )
        parser.add_argument(
            "--output-dir",
    ...
    def build_parser() -> argparse.ArgumentParser:
        ...
        p.add_argument("--real", action="store_true",
                       help="Use real TinyStories (requires internet). Default: synthetic data.")
    ...
    Both scripts use `train=True` which triggers `TrainingRunner(mode="local", ...)` by default.
    When the trainer config is in mock mode the test suite remains fast.
  ...
  <done>
    Both scripts import without error. `--help` works for both. No circular imports. Loss function shapes are correct (view(-1, VOCAB_SIZE)).
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    - GPT has n() method returning positive int
    ...
    class TestGPTContract:
        ...
        def test_n_method_exists(self):
            """GPT.n() exists and returns a positive integer."""
        ...
        def test_n_increases_with_d_model(self):
            """Larger d_model produces larger n()."""
            from flops_fit.examples import GPT, GPTConfig
    ...
    class TestProgrammaticExample:
        ...
        def test_make_model_factory_creates_gpt_with_n(self):
            """make_model_factory returns a callable that produces GPT with n()."""
    ...
    class TestCLIWrapperExample:
        ...
        def test_make_model_factory(self):
            """make_model_factory in CLI wrapper creates GPT with n()."""
  ...
  <verify>
    ...
    - TestGPTContract (3 tests)
    - TestTinyStoriesDatasetLazyLoad (3 tests)
    - TestTinyStoriesDatasetWithMock (4 tests)
    - TestProgrammaticExample (3 tests)
    - TestCLIWrapperExample (3 tests)

    Run: `python -m pytest tests/ -x -q` — all 169 + new tests pass.
  </verify>
  <done>
    All tests in test_examples.py pass. Full suite (169 + new) passes. No HuggingFace network calls in CI. GPT contract (n() method) verified via tests.
  </done>
</task>
...
2. `test_examples.py` covers GPT contract, TinyStories lazy-loading, mocked dataset, and example script smoke tests.
3. No HuggingFace downloads triggered during `pytest tests/test_examples.py`.
4. Full test suite passes without regressions.
5. Phase 7 success criteria satisfied:
   ...
   - Examples produce output in mock mode.
</success_criteria>
