...
must_haves:
  truths:
    - "find_optimal() generates a sweep plan when compute_budgets are provided"
...
2. After loss_fn validation, add sweep planning block:
   ```python
   ...
   ```
3. Update NotImplementedError message to mention "planning passed" when compute_budgets were provided:
   - With budgets: "find_optimal() validation and sweep planning passed. Training engine not yet implemented."
   - Without budgets: Keep existing message "find_optimal() model validation passed. Full pipeline not yet implemented."
...
Run `python -c "from flops_fit import SweepPlan, Experiment, plan_sweep; print('exports ok')"`. Run `python -m pytest tests/test_api.py -v` to verify existing tests still pass (the NotImplementedError message match in existing tests needs updating since we changed the message -- check and update test match strings if needed).
  ...
  <done>
find_optimal() calls plan_sweep() when compute_budgets are provided. SweepPlan, Experiment, and plan_sweep are importable from flops_fit.
  </done>
</task>
...
1. **test_accepts_compute_budgets**: Call `find_optimal(MockModel, "width", model_kwargs={"num_layers": 4}, compute_budgets=[1e15, 1e16])`. Assert raises NotImplementedError with "sweep planning passed" in message (confirming planning ran).

2. **test_no_compute_budgets_backward_compat**: Call `find_optimal(MockModel, "width")` without compute_budgets. Assert raises NotImplementedError with "model validation passed" (original message, confirming no sweep planning ran).

3. **test_compute_budgets_none_backward_compat**: Call `find_optimal(MockModel, "width", compute_budgets=None)`. Same assertion as above.
...
Also update existing test_find_optimal_validates_then_raises_not_implemented if the error message changed -- match the new message pattern so existing tests don't break.
  </action>
  <verify>
...
<success_criteria>
find_optimal() generates a sweep plan when compute_budgets are provided, SweepPlan is a top-level export, validation order is enforced, and backward compatibility is preserved.
</success_criteria>
